/*
 * v.0 - original: https://github.com/embeddedlab786/Real_Time_Clock_based_4Load_Control/tree/main
 * v.1 - some changes: https://forum.arduino.cc/t/control-4-relays-to-turn-on-for-a-few-seconds-a-day-and-then-turn-off/1112044/15
 * v.1a - Nicu FLORICA (niq_ro) - small changes to recover status after restart
 * v.2- changed display on i2c + added serial info for easy control of the status
 * v.2.a - added rest of control for outputs
 * v.2.b - removed serial info 
 */
#include <DS3231.h>//Memanggil RTC3231 Library - https://electronoobs.com/eng_arduino_ds3231.php
#include <EEPROM.h>
...
unsigned int Start1HHMM, Start2HHMM, Start3HHMM, Start4HHMM; 
unsigned int Finish1HHMM, Finish2HHMM, Finish3HHMM, Finish4HHMM;
unsigned int ceasHHMM;

byte timer1, timer2, timer3, timer4;

byte stop =0, mode=0, flag=0;
/*
// chinesse relay control
#define atras LOW
#define repaus HIGH
*/
// simple led for test or normal relay control
#define atras HIGH
#define repaus LOW
...
pinMode(relay1, OUTPUT);digitalWrite(relay1, repaus);
pinMode(relay2, OUTPUT);digitalWrite(relay2, repaus);
pinMode(relay3, OUTPUT);digitalWrite(relay3, repaus);
pinMode(relay4, OUTPUT);digitalWrite(relay4, repaus);
...
Start1HHMM = 100*Start1HH + Start1MM;
Start2HHMM = 100*Start2HH + Start2MM;
Start3HHMM = 100*Start3HH + Start3MM;
Start4HHMM = 100*Start4HH + Start4MM;
Finish1HHMM = 100*Finish1HH + Finish1MM;
Finish2HHMM = 100*Finish2HH + Finish2MM;
Finish3HHMM = 100*Finish3HH + Finish3MM;
Finish4HHMM = 100*Finish4HH + Finish4MM;
ceasHHMM = 100*hh + mm;
...
{ 
  if (timer1 == 1)
  {
  if (Start1HHMM < Finish1HHMM)
   {
    if ((ceasHHMM >= Start1HHMM) and (ceasHHMM < Finish1HHMM))
    {
    digitalWrite(relay1, atras);
    }
    else
    {
    digitalWrite(relay1, repaus);  
    }
   }
   else
   {
  ...
  if (timer2 == 1)
  {
  if (Start2HHMM < Finish2HHMM)
   {
    if ((ceasHHMM >= Start2HHMM) and (ceasHHMM < Finish2HHMM))
    {
    digitalWrite(relay2, atras);
    }
    else
    {
    digitalWrite(relay2, repaus);  
    }
   }
   else
   {
  ...
  if (timer3 == 1)
  {
  if (Start3HHMM < Finish3HHMM)
   {
    if ((ceasHHMM >= Start3HHMM) and (ceasHHMM < Finish3HHMM))
    {
    digitalWrite(relay3, atras);
    }
    else
    {
    digitalWrite(relay3, repaus);  
    }
   }
   else
   {
  ...
  if (timer4 == 1)
  {
  if (Start4HHMM < Finish4HHMM)
   {
    if ((ceasHHMM >= Start4HHMM) and (ceasHHMM < Finish4HHMM))
    {
    digitalWrite(relay4, atras);
    }
    else
    {
    digitalWrite(relay4, repaus);  
    }
   }
   else
   {
...
if (timer1 == 0) digitalWrite(relay1, repaus);
if (timer2 == 0) digitalWrite(relay2, repaus);
if (timer3 == 0) digitalWrite(relay3, repaus);
if (timer4 == 0) digitalWrite(relay4, repaus);
